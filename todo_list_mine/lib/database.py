...
class Connection:
    ...
    def create_user(self, login, password):
        ...
        if lst:
            ...
            password = enc.encoding(
                self.count_users()+1, str(password), "encode")
            self.cursor.execute(
    ...
    def insert_to_do(self, user_id, text, status):
        self.cursor.execute(
            f'''INSERT INTO todos(user_id,content,status) VALUES ({user_id},"{text}","{status}");''')
        self.connection.commit()
        return (self.select_all())

    def get_to_dos(self, user_id):
        self.cursor.execute(
            f'''SELECT * FROM todos WHERE status <>"closed" and user_id = {user_id};''')
        return (self.cursor.fetchall())

    def update_to_do_status_to_close(self, id):
        self.cursor.execute(
            f'''UPDATE todos SET status="closed" WHERE id={id}''')
        self.connection.commit()
    ...
    def update_to_do(self, id, text):
        self.cursor.execute(f'''UPDATE todos SET content = "{text}" WHERE id = {id} ''')
        self.connection.commit()
